    ...
    public sealed class ThemeManager
    {
        ...
        public Theme GetTheme(Options options)
        {
            ...
            if (themesDir.Exists)
            {
                ...
                if (themeDir.Exists)
                {
                    FileInfo file = new FileInfo(themeDir.FullName + @"\theme2.xml");
                    if (file.Exists)
                    {
                        theme = getThemeFromXml(file.FullName);
                    }
                }
            }
            return theme;
        ...
        private Theme getThemeFromXml(string filename)
        {
            ...
            if (reader.NodeType == XmlNodeType.Element)
            {
                if (reader.IsStartElement())
                {
                    ...
                    else if (reader.Name.Equals("Board"))
                    {
                        ...
                        getBoadFromXml(reader, ref themeBoard);
        ...
        private void getBoadFromXml(XmlReader reader, ref ThemeBoard returnBoard)
        {
            while (reader.Read())
            {
        ...
        private void processBoardElement(XmlReader reader, ref ThemeBoard returnBoard)
        {
            if (reader.NodeType == XmlNodeType.Element)
            {
                if (reader.IsStartElement())
                {
                    ...
                    else if (reader.Name.Equals("BackgroundImage"))
                    {
                        reader.Read();
                        returnBoard.BackgroundImageName = getStringValue(reader);

                        verifyAndLoadBackgroundImage(ref returnBoard);
                    }
        ...
        private void verifyAndLoadBackgroundImage(ref ThemeBoard board)
        {
            string fileName = gameOptions.ThemesFolder + gameOptions.ThemeName + @"\" + board.BackgroundImageName;
            FileInfo file = new FileInfo(gameOptions.ThemesFolder + gameOptions.ThemeName + @"\" + fileName);
            if (file.Exists)
            {
                Image bg = Image.FromFile(file.FullName);
                board.BackgroundImage = bg;
            }
            else
            {
                throw new ApplicationException(string.Format("Background image {0} not found", fileName));
            }
        }
